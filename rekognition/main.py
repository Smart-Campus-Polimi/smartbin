...
def takePhoto(my_cam, name):
    ...
    file_name = os.path.expanduser(DIRECTORY) + name + "_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + str(
        ".png")
    ...
    while (is_running):
        ...
        if (isPhoto):
            ...
            with open(FILE_NAME, 'a') as csvfile:
                ...
                filewriter.writerow(["{0:.4f}".format(photoT), "{0:.4f}".format(requestT), "{0:.4f}".format(labelingT),
                                     "{0:.4f}".format(totalT)])
            isPhoto = False
            cheat_waste = None
    # time.sleep(1)
